  ...
  return {
    ...
    visitor: {
      "ImportDeclaration|VariableDeclaration": function ImportDeclarationVariableDeclaration(path, state) {
        var optionName = state.opts.optionName;

        var TARGET_PKG_NAME = optionName || "custom-antd-resize-table";
        //排除自定义Table组件的import
        if (state.file.opts.filename && state.file.opts.filename.includes(TARGET_PKG_NAME)) {
          return;
        }
